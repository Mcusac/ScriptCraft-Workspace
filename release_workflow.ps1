...
Write-Host "🚀 ScriptCraft Release Workflow Automation" -ForegroundColor Green
...
if ($UpdateVersion -eq "y" -or $UpdateVersion -eq "Y") {
    ...
    if ($NewVersion -match '^\d+\.\d+\.\d+$') {
        ...
        Write-Host "❌ Invalid version format. Please use format: X.Y.Z (e.g., 1.3.4)" -ForegroundColor Red
        exit 1
    }
}
...
Write-Host "`n📦 Step 1: Updating python-package submodule..." -ForegroundColor Yellow

Set-Location "implementations/python-package"

# Step 1a: Update version if requested
if ($NewVersion) {
    Write-Host "`n📝 Updating version from $CurrentVersion to $NewVersion..." -ForegroundColor Yellow
    ...
    Write-Host "✅ Version updated in scriptcraft/_version.py" -ForegroundColor Green
}
...
if ($status) {
    ...
    Write-Host "✅ Submodule updated successfully!" -ForegroundColor Green
} else {
    Write-Host "ℹ️  No changes detected in submodule" -ForegroundColor Yellow
}

# Step 2: Update main workspace repository
Write-Host "`n🏠 Step 2: Updating main workspace..." -ForegroundColor Yellow
...
if ($status -match "implementations/python-package") {
    ...
    Write-Host "✅ Workspace updated successfully!" -ForegroundColor Green
} else {
    Write-Host "ℹ️  No submodule updates detected" -ForegroundColor Yellow
}

# Step 3: Optional version tagging
if ($Version) {
    Write-Host "`n🏷️  Step 3: Creating version tag..." -ForegroundColor Yellow
    Set-Location "implementations/python-package"
    ...
    Write-Host "✅ Version tag v$Version created and pushed!" -ForegroundColor Green
}
