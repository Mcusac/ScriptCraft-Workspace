"""
Clean MLP v3 trainer with sparse label support for CAFA 6.

Key improvements over v1/v2:
- Uses sparse tensors throughout (no dense conversion)
- Custom sparse-aware BCE loss (computes loss only on labeled indices)
- Simple, clean architecture following KISS principle
- No memory workarounds (memmap, streaming, etc.)
...
- Sparse approach (v3): 6.3MB → fits easily
"""
...
def train_ontology_model(X_train: Union[np.ndarray, Path], y_train: csr_matrix,
    ...
    """
    Train MLP v3 model for a specific ontology using sparse labels.
    ...
    """
    log_training_start(ont_name, "MLP v3")
...
def train_all_ontologies(X_train: np.ndarray, y_train_dict: Dict[str, csr_matrix],
    ...
    """
    Train MLP v3 models for all ontologies.
