...
class BaseTool(BaseComponent):
    ...
    @staticmethod
    def is_shippable_environment() -> bool:
        """
        Detect if we're running in a shippable environment.
        DRY method for environment detection across all tools.
        
        Returns:
            bool: True if in shippable environment, False if in development
        """
        current_dir = Path.cwd()
        return (current_dir.name == 'scripts' or 
                'shippable' in str(current_dir).lower())
    
    def resolve_input_directory(self, **kwargs) -> Path:
        """
        Resolve input directory path for both development and shippable environments.
        ...
        if self.is_shippable_environment():
            current_dir = Path.cwd()
...
class BaseMainRunner:
    ...
    def setup_environment() -> None:
        """Set up the Python path for both dev and shippable environments."""
    ...
    def import_tool(tool_name: str) -> Any:
        ...
        try:
            ...
            try:
                # Fallback for shippable environment  
