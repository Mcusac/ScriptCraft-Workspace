...
from scripts.tools.tool_dispatcher import ToolRegistry
...
def mock_tool_dir(temp_dir):
    ...
    with open(tool1_dir / "main.py", 'w') as f:
        f.write("""
from scripts.common.base_tools import BaseTool
...
def test_tool_registry_discover_tools(mock_tool_dir, monkeypatch):
    """Test tool discovery."""
    monkeypatch.setattr('scripts.tools.tool_dispatcher.TOOLS_FOLDER', mock_tool_dir)
...
def test_tool_registry_get_tool(mock_tool_dir, monkeypatch):
...
def test_tool_registry_list_tools(mock_tool_dir, monkeypatch):
...
def test_tool_registry_caching(mock_tool_dir, monkeypatch):
